...
% Updated July 2024
%--------------------------------------------------------------------------
% Difference here: gives you an option as to whether to proceed.
% Helpful for efficiency when boundary tracking is just good...
function [thisx,thisy] = func_manual_zoomout(pic,datax,datay,fr)
% INPUT
...
% thisx, thisy - edited datapoints
    ...
    %% New step: add whether you would like to edit:
    counter = 0;
    while true
        %% Delete Points
        ...
        counter = counter + 1;
        cont = input('Proceed with manual point editing?: ');
        if cont == 0
            if counter == 1
                thisx = datax;
                thisy = datay;
        ...
        % Bobae: weird bug fixing with array concatenation
        sizedatax = size(datax); sizeaddx = size(addx);
        if sizedatax(2) ~= sizeaddx(2)
